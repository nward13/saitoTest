...
contract SimplePaymentChannel {
    ...
    // block delay
    //
    // CHANGED FOR TESTING: shorten block delay so test can run faster
    // Originl block_delay = 5760 (24 hour at 15 secs)
    uint block_delay = 5; 
    ...
    //
    function balance(uint channel_id, uint uid) view returns (uint) {
    ...
    function withdraw(uint channel_id, uint withdrawal1, uint sig1, bytes32 r1, bytes32 s1, uint withdrawal2, uint sig2, bytes32 r2, bytes32 s2) returns (uint) {
      ...
      // CHANGED FOR TESTING: validateMessage() should take party 2's withdrawal approval signed by party 1,
      // and party 1's withdrawal signed by party 2
      var msg = generateMessage(withdrawal1, sig1);
      valid = validateMessage(address2[channel_id], msg, r1, s1);
      if (valid == 0) { return 0; }

      msg = generateMessage(withdrawal2, sig2);
      valid = validateMessage(address1[channel_id], msg, r2, s2);
      if (valid == 0) { return 0; }
    ...
    function validateMessage(address add1, bytes32 m, bytes32 r, bytes32 s) returns (uint) {
        ...
        // CHANGED FOR TESTING
        // ecrecover requires header to be added
        bytes memory header = "\x19Ethereum Signed Message:\n32";
        m = keccak256(header, m);
